...
public class SsoResourceImpl implements SsoResource {
  ...
  public CompletionStage<Response> logout(String sessionId) {
    return ssoService.logout(sessionId)
        .thenApply(deleted -> {
          ResponseBuilder builder = deleted ? Response.noContent()
              : DataResponse.error(Boom.badRequest().message("Session does not exist")).builder();
  ...
  public CompletionStage<Response> session(String sessionId) {
    return ssoService.findSession(sessionId)
        .thenApply(maybeUser -> {
