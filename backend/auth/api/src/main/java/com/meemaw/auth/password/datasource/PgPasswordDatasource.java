...
public class PgPasswordDatasource implements PasswordDatasource {
  ...
  private static final String CREATE_RAW_SQL = "INSERT INTO auth.password(user_id, hash) VALUES($1, $2) RETURNING *";

  @Override
  public CompletionStage<Boolean> create(Transaction transaction, UUID userId,
      String hashedPassword) {
    Tuple values = Tuple.of(userId, hashedPassword);
    return transaction.preparedQuery(CREATE_RAW_SQL, values).thenApply(pgRowSet -> {
